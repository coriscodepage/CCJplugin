...
import { Player, PlayerResponse } from '../models/player'
...
export const saveUserdata: EPR = async (info, data, send) => {
    ...
    const saveData: Player = {collection: 'profile', name: uname, pdata: $(data).element('pdata').obj, gachaitem: $(data).content('gachaitem')};
    console.log(uname)
    ...
    console.log(refid)
    await DB.Upsert<Player>(refid, {collection: 'profile'}, saveData);
    send.success();
}
...
export const getUserData: EPR = async (info, data, send) => {
    ...
    console.log(util.inspect(data, {showHidden: false, depth: null, colors: true}));
    const refid: string = $(data).content('ref_id');
    const profile = await DB.FindOne<Player>(refid, {collection: 'profile'});
    let dataObj = [];
